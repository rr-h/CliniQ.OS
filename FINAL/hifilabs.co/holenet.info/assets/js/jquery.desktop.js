  ...
  return {
    ...
    init: {
      ...
      desktop: function() {
		...
		d.on('mousedown', 'a.winlink', function() {
          // Highlight the icon.
          JQD.util.clear_active();
          $(this).addClass('active');
        });
        ...
        d.on('dblclick', 'a.winlink', function() {
          // Get the link's target.
          var x = $(this).attr('href');
          var y = $(x).find('a').attr('href');
        ...
        d.on('dblclick', 'div.window_top_resize', function() {
      ...
      wallpaper: function() {
		...
		/* var randombgimg = Math.floor((Math.random() * 361));
        if ($('#desktop').length) {
          $('body').prepend('<img id="wallpaper" class="abs" src="assets/images/misc/bg/' + randombgimg + '.PNG" style="background-repeat: repeat;" />');
        } */
      }
    },
    util: {
      ...
      window_resize: function(el) {
        ...
        var win = $(el).closest('div.window_resizable');
