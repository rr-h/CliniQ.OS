...
function solitaireGame(oUniqueName,oWorkspace,oDeck,oWastePile,oFoundation,oTableau,oCardWord,oCardNames,oSuits,youWin,oDeal,oOptions,
	...
	this.cardStacks = [];
	// Deck
	this.cardStacks[0] = new cardStack(0,0,true,'backo');
	// Waste Pile
	this.cardStacks[1] = new cardStack(1,1,true,'cardbase');
	// Foundation
	for( var i = 2; i < 6; i++ ) {
		this.cardStacks[i] = new cardStack(2,i,true,'cardbase');
	}
	// Tableau
	for( var i = 6; i < 13; i++ ) {
		this.cardStacks[i] = new cardStack(3,i,true,'cardbase');
	}
...
solitaireGame.prototype.handleClicksOnCards = function (e) {
	...
	if( this.relatedObject.game.doingSpatNav ) {
		...
		while( tmpCard ) {
			...
			tmpCard.representation.style.opacity = '0.8';
			tmpCard.representation.style.MozOpacity = '0.8';
			tmpCard.representation.style.filter = 'alpha(opacity=80)';
			if( tmpCard.representation.style.setProperty ) {
				// Safari 1.1
				try { tmpCard.representation.style.setProperty('-khtml-opacity','0.8'); } catch(e) {}
			}
...
solitaireGame.prototype.showPrefs = function (e) {
	...
	var prfHead = document.createElement('span');
	prfHead.style.color = "#FFF";
	prfHead.appendChild(document.createTextNode(this.strings.prefName));
	var imgHead = document.createElement('span');
	imgHead.style.color = "#FFF";
	imgHead.appendChild(document.createTextNode(this.strings.imgPref+this.usingCards.width));
	var dealHead = document.createElement('span');
	dealHead.style.color = "#FFF";
	...
	firstLabel.style.color = "#FFF";
	var secndLabel = document.createElement('label');
	secndLabel.style.color = "#FFF";
